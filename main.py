import axiom_extraction
import time
import correlation_detection
import influence_detection
import os
import utils
from utils import *
import sys
...
def prepare_data(model_dataset,model_name,prefix,user_th_value):
...
def predict_all(model_dataset,model_name,user_th,prefix):
    rel_dict, ent_dict, known_facts, predictions, type, th_value, e_clusters, r_labels = prepare_data(
        model_dataset, model_name, prefix, user_th)
    fact_explaination={}
...
def predict_single(fact, model_dataset, model_name, user_th, goal, prefix):
    rel_dict, ent_dict, known_facts, _, type, th_value, e_clusters, r_labels = prepare_data(
        model_dataset, model_name, prefix, user_th)
    h = fact[0]
    cur_rel = fact[1]
    t = fact[2]
...
if __name__=='__main__':
    ...
    fact_explaination={}
    if fact:
        fact_explaination[(goal, fact[0], fact[1], fact[2])]=predict_single(fact,model_dataset,model_name,user_th,goal,prefix)

    elif all:
        fact_explaination=predict_all(model_dataset,model_name,user_th,prefix)
